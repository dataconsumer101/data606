...
* A streak length starts with the first attempt at the start of the game or after another streak.
* A streak always ends with a miss
* A streak of one means one hit before a miss
* A streak of zero mean a miss without a hit
...
* Streaks aren't common, most of the time they are streaks of 0, which means lots of misses
* Right skewed
* Longest streak was 4 hits before a miss
...
* We need to add the probabilty paramter:
...
```{r, eval = T}
sample_streak <- calc_streak(sim_basket)
barplot(table(sample_streak))
```
...
* The distribution for the random sample seems like a more gradual decline, with fewer zero streaks. 
* The random sample seems to perform in a similar way, with a generally similiar distribution.
* longest streak was 4.
...
* I would expect the streak to look similiar, with the most zeros and with a right skew. This is probably because hot hands isn't a real thing.
...
library(dplyr)
library(ggplot2)
df <- bind_rows(data.frame(grp = 'kobe', streak = kobe_streak, stringsAsFactors = F),
                data.frame(grp = toString('sample'), streak = sample_streak, stringsAsFactors = F)) %>%
  group_by(grp, streak) %>%
  summarize(count = n())

ggplot(df, aes(x = streak, y = count, fill = grp)) +
  geom_col(position = 'dodge')

```

* It looks like the random sample performed similarly well, with fewer zero streaks and almost twice as may 2 shot streaks.
* Interestingly, both had the same number of 3 and 4 shot streaks, but still very few of both.
